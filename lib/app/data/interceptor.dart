    ...
    if (response.isOk) {
      if (res is Map &&
        ...
        if (res["errors"]["error"] != null &&
            res["errors"]["error"].toString().isNotEmpty) {
          showError(ApiError(
            type: ErrorType.response,
            error: res["errors"]["error"]?.toString() ?? Strings.unknownError,
          ));
        } else {
          showError(ApiError(
            type: ErrorType.response,
      ...
      if (status.isServerError) {
        ...
        showError(ApiError(
          ...
          error: res["errors"]["error"]?.toString() ?? Strings.unauthorize,
        ));
      } else {
        // final ErrorModel model = ErrorModel.fromJson(res);
        final error = res["errors"]["error"];
